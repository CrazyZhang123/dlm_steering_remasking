...
2026-07-03 补充：M12 附近细扫 `M = 9/10/11`（每点固定同一 bank 重复 3 轮推理+评判，
详见 `docs/stage2_ct_csd_m_scan_9_10_11_log.md`）。因推理天然非确定，单次 ASR 噪声约
±2.5pp，该细扫以 3 轮均值/极差判断：

| `num_total_clusters` | 方法 | 3 轮 ASR | ASR 均值 | ASR 极差 | `activation_rate` 3 轮均值 | 判断 |
|---:|---|---|---:|---:|---:|---|
| `9` | CT-CSD M9 | 70/72/65 | `69.0%` | 7 | `0.2198` | 与 M12 三次重跑均值 67.7 持平（+1.3，噪声内），无优势 |
| `10` | CT-CSD M10 | 71/70/73 | `71.3%` | 3 | `0.2383` | 高于 M12（+3.6pp），更差 |
| `11` | CT-CSD M11 | 73/71/71 | `71.7%` | 2 | `0.2439` | 高于 M12（+4.0pp，超出噪声带），稳定更差 |

细扫结论：`M9/M10/M11` 均未优于 M12（M12 同配置重跑 3 次 ASR = 65/70/68，均值 67.7），
且 9→10→11 呈轻微单调变差趋势；`num_total_clusters = 12` 维持为阶段 1 簇数消融的
最优点位与默认 `M*` 候选。
...
## 阶段 5：Feature preprocessing 消融（进行中，截至 2026-07-03 16:38）

Stage 5 验证在 route/聚类特征上做 `center` / `PCA` 预处理是否改善 ASR。全部在全局 `ct_csd M12`
base、100 样本 JBB+DIJA、本地 Llama-Guard、每点 3 轮口径下进行。专题日志见
`docs/stage5_m12_preprocess_ablation_log.md` 与 `docs/stage5_ct_csd_pca128_m_scan_12_9_10_11_log.md`。

| 预处理 | 维度 | ASR 三轮 | 均值 | vs 最优 l2_only(67.7) | 状态 |
|---|---:|---|---:|---:|---|
| `l2_only` | 4096 | 65/70/68 | 67.7 | 基准（全项目最低点） | 已完成 |
| `center_pca128_l2` | 128 | 70/70/72 | 70.7 | +3.0（更差） | 已完成 |
| `center_l2` | 4096 | 72/71/74 | 72.3 | +4.6（更差） | 已完成 |
| `center_pca256_l2` | 256 | — | — | — | bank 构造中（pass1 47%，2026-07-03 16:38） |

**当前判断**：三个已完成点位一致排序 `l2_only 67.7 < pca128 70.7 < center_l2 72.3`。`center_l2`
单独只做去均值就 +4.6pp，说明**劣化主要来自"去均值"本身**（成功拆变量归因）。`center_pca256_l2`
仍在 GPU1 构造（已运行 ~1h09m，pass 1 进度 47%，后续 pass 2 + 3 轮推理评判还需数小时），
预期同样更差。

**总览结论更新**：截至目前，`bank 表示 / token 选择 / 簇数 / 特征预处理` 四条线全部无正收益，
全项目最低 ASR 仍是最朴素的 `ct_csd M12 + l2_only + all token = 67.7%`。后续降 ASR 建议转向
**从未开发的 steering 超参与重掩码机制**，详见 `docs/plan/asr_reduction_improvement_plan.md`。
