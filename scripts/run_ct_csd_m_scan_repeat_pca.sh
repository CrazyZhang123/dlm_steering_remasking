#!/usr/bin/env bash
# Stage 5 降维对应版簇数细扫（全局 ct_csd, token_selection=all, feature_preprocess=center_pca128_l2）：
# 与 scripts/run_ct_csd_m_scan_repeat.sh 完全同口径，唯一差异是 bank 构造加 PCA128 白化路由特征
# （bank 内嵌 preprocess 状态，推理端 CTCSDBank.route() 自动在 PCA 空间路由，推理命令不变）。
# 独立成文件的原因：原脚本正在 tmux 中运行，不修改运行中的脚本。
# 用法: bash scripts/run_ct_csd_m_scan_repeat_pca.sh <gpu_id> <n_rounds> <m1> [m2 ...]
set -euo pipefail
...
PREPROCESS="center_pca128_l2"
SUFFIX="pca128"

MASTER_LOG="outputs/stage5_ct_csd_${SUFFIX}_m_scan_gpu${GPU_ID}.log"
mkdir -p outputs

{
  date -Is
  echo "[scan] gpu=${GPU_ID} rounds=${N_ROUNDS} m_list=$* preprocess=${PREPROCESS}"
  for M in "$@"; do
    BANK_DIR="outputs/ct_csd_llada_m${M}_${SUFFIX}"
    if [[ -f "${BANK_DIR}/ct_csd_bank.pt" ]]; then
      ...
      mkdir -p "${BANK_DIR}"
      date -Is
      echo "[M=${M}] build ct_csd bank (${PREPROCESS})"
    ...
    for R in $(seq 1 "${N_ROUNDS}"); do
      EVAL_DIR="outputs/jbb_dija_ct_csd_m${M}_${SUFFIX}_r${R}"
